
import React from 'react';
import { motion, useScroll, useTransform } from 'framer-motion';
import { GraduationCap, CheckCircle, BookOpen, Award, FileText, AlertCircle, Sparkles, Users } from 'lucide-react';

const EligibilityCriteria: React.FC = () => {
  const { scrollYProgress } = useScroll();
  const y1 = useTransform(scrollYProgress, [0, 1], [0, -100]);
  const y2 = useTransform(scrollYProgress, [0, 1], [0, 100]);

  return (
    <div className="bg-white min-h-screen selection:bg-[#ff6b35]/20 selection:text-[#ff6b35] pt-[180px]">{/* Fixed navbar padding */}
      {/* Immersive Hero Section */}
      <section className="relative h-[60vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <motion.img 
            style={{ scale: 1.1, y: y1 }}
            src="/images/eligibility_hero.jpg" 
            alt="Eligibility Criteria" 
            className="w-full h-full object-cover brightness-[0.4]"
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[#004a99]/40 via-transparent to-white" />
          
          {/* Floating Glass Particles */}
          <div className="absolute inset-0 pointer-events-none">
            {Array.from({ length: 5 }).map((_, i) => (
              <motion.div
                key={i}
                animate={{ 
                  y: [0, -20, 0],
                  rotate: [0, 10, 0]
                }}
                transition={{ 
                  duration: 5 + i, 
                  repeat: Infinity, 
                  ease: "easeInOut" 
                }}
                className="absolute w-24 h-24 border border-white/10 bg-white/5 backdrop-blur-sm rounded-2xl"
                style={{ 
                  top: `${20 + i * 15}%`, 
                  left: `${10 + i * 20}%`,
                  opacity: 0.3
                }}
              />
            ))}
          </div>
        </div>

        <div className="container mx-auto px-6 relative z-10 text-center">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
          >
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white text-[10px] font-black uppercase tracking-[0.3em] mb-6">
              <Sparkles size={14} className="text-[#ff6b35]" />
              Admission Requirements
            </span>
            <h1 className="text-7xl md:text-9xl font-black text-white tracking-tighter drop-shadow-2xl">
              Eligibility <span className="text-[#ff6b35]">Criteria</span>
            </h1>
            <p className="text-white/80 text-xl md:text-2xl font-medium mt-6 max-w-3xl mx-auto">
              Your pathway to engineering excellence starts here
            </p>
          </motion.div>
        </div>
      </section>

      {/* B.E./B.Tech Programs Section */}
      <section className="py-32 relative -mt-32 z-20">
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            
            {/* Left Column: Image */}
            <motion.div 
              className="lg:col-span-5 relative"
              initial={{ opacity: 0, x: -50 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              style={{ y: y2 }}
            >
              <div className="relative p-1 bg-gradient-to-br from-[#004a99] to-[#0066cc] rounded-[3rem] shadow-2xl overflow-hidden group">
                <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
                <img 
                  src="/images/btech_requirements.jpg" 
                  alt="B.Tech Requirements" 
                  className="w-full h-[500px] object-cover rounded-[2.8rem] transition-transform duration-1000 group-hover:scale-110"
                />
                
                {/* Floating Glass Data Card */}
                <div className="absolute bottom-10 -right-10 water-glass p-8 bg-white/95 backdrop-blur-2xl border-white shadow-3xl max-w-[280px]">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 bg-[#004a99] rounded-xl flex items-center justify-center text-white">
                      <GraduationCap size={24} />
                    </div>
                    <h4 className="text-lg font-black text-[#004a99] tracking-tight">10+2 Required</h4>
                  </div>
                  <p className="text-slate-500 text-sm font-medium leading-relaxed">
                    Physics, Chemistry, Mathematics mandatory
                  </p>
                </div>
              </div>
            </motion.div>

            {/* Right Column: Content */}
            <motion.div 
              initial={{ opacity: 0, x: 50 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              className="lg:col-span-7"
            >
              <div className="water-glass !bg-slate-50/50 p-12 md:p-20 border-white/80 shadow-[0_40px_100px_-20px_rgba(0,74,153,0.1)] relative overflow-hidden">
                {/* Refractive Watermark */}
                <div className="absolute top-0 right-0 p-10 opacity-[0.03] pointer-events-none">
                  <GraduationCap size={300} strokeWidth={1} />
                </div>

                <div className="relative z-10">
                  <div className="flex items-center gap-3 text-[#004a99] mb-8">
                    <div className="w-12 h-1 bg-[#ff6b35] rounded-full" />
                    <span className="font-black uppercase tracking-[0.2em] text-xs">Undergraduate Programs</span>
                  </div>

                  <h2 className="text-5xl md:text-7xl font-black text-slate-900 mb-10 tracking-tighter leading-[0.9]">
                    B.E. / B.Tech <br />
                    <span className="text-[#004a99]">Programs</span>
                  </h2>

                  <div className="space-y-6 text-slate-600 text-lg font-medium leading-relaxed max-w-2xl">
                    <p className="relative pl-8 border-l-4 border-[#ff6b35]">
                      Candidates must have passed 10+2 examination with Physics, Chemistry, and Mathematics as core subjects with minimum 50% aggregate marks.
                    </p>
                  </div>

                  {/* Requirements Checklist */}
                  <div className="mt-10 space-y-4">
                    {[
                      'Passed 10+2 or equivalent examination',
                      'Physics, Chemistry, Mathematics mandatory',
                      'Minimum 50% aggregate marks (45% for reserved categories)',
                      'Valid TNEA counseling rank or JEE Main score',
                      'Age limit: Minimum 17 years as on 31st December'
                    ].map((req, i) => (
                      <motion.div
                        key={i}
                        initial={{ opacity: 0, x: -20 }}
                        whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: i * 0.1 }}
                        className="flex items-start gap-4"
                      >
                        <div className="w-6 h-6 rounded-full bg-[#004a99]/10 flex items-center justify-center flex-shrink-0 mt-1">
                          <CheckCircle size={16} className="text-[#004a99]" />
                        </div>
                        <p className="text-slate-700 font-medium">{req}</p>
                      </motion.div>
                    ))}
                  </div>
                </div>
              </div>
            </motion.div>

          </div>
        </div>
      </section>

      {/* Lateral Entry (Diploma) Section - Reversed Layout */}
      <section className="py-32 bg-slate-50">
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            
            {/* Left Column: Content */}
            <motion.div 
              initial={{ opacity: 0, x: -50 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              className="lg:col-span-7"
            >
              <div className="water-glass !bg-white/50 p-12 md:p-20 border-white/80 shadow-[0_40px_100px_-20px_rgba(0,74,153,0.1)] relative overflow-hidden">
                <div className="absolute top-0 left-0 p-10 opacity-[0.03] pointer-events-none">
                  <Award size={300} strokeWidth={1} />
                </div>

                <div className="relative z-10">
                  <div className="flex items-center gap-3 text-[#004a99] mb-8">
                    <div className="w-12 h-1 bg-[#ff6b35] rounded-full" />
                    <span className="font-black uppercase tracking-[0.2em] text-xs">Lateral Entry</span>
                  </div>

                  <h2 className="text-5xl md:text-7xl font-black text-slate-900 mb-10 tracking-tighter leading-[0.9]">
                    Diploma <br />
                    <span className="text-[#004a99]">Holders</span>
                  </h2>

                  <div className="space-y-6 text-slate-600 text-lg font-medium leading-relaxed max-w-2xl">
                    <p className="relative pl-8 border-l-4 border-[#ff6b35]">
                      Diploma holders can get direct admission to the second year (3rd semester) of B.E./B.Tech programs through lateral entry scheme.
                    </p>
                  </div>

                  {/* Requirements Checklist */}
                  <div className="mt-10 space-y-4">
                    {[
                      'Passed 3-year Diploma in relevant engineering branch',
                      'Minimum 50% aggregate marks in diploma (45% for reserved)',
                      'Valid TNEA lateral entry counseling rank',
                      'Diploma from AICTE approved institution',
                      'Direct entry to 2nd year (3rd semester)'
                    ].map((req, i) => (
                      <motion.div
                        key={i}
                        initial={{ opacity: 0, x: -20 }}
                        whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: i * 0.1 }}
                        className="flex items-start gap-4"
                      >
                        <div className="w-6 h-6 rounded-full bg-[#ff6b35]/10 flex items-center justify-center flex-shrink-0 mt-1">
                          <CheckCircle size={16} className="text-[#ff6b35]" />
                        </div>
                        <p className="text-slate-700 font-medium">{req}</p>
                      </motion.div>
                    ))}
                  </div>
                </div>
              </div>
            </motion.div>

            {/* Right Column: Image */}
            <motion.div 
              className="lg:col-span-5 relative"
              initial={{ opacity: 0, x: 50 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
            >
              <div className="relative p-1 bg-gradient-to-br from-[#ff6b35] to-[#ff9e7d] rounded-[3rem] shadow-2xl overflow-hidden group">
                <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
                <img 
                  src="/images/diploma_lateral_entry.jpg" 
                  alt="Diploma Lateral Entry" 
                  className="w-full h-[500px] object-cover rounded-[2.8rem] transition-transform duration-1000 group-hover:scale-110"
                />
              </div>
            </motion.div>

          </div>
        </div>
      </section>

      {/* M.E./M.Tech Programs Section */}
      <section className="py-32">
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            
            {/* Left Column: Image */}
            <motion.div 
              className="lg:col-span-5 relative"
              initial={{ opacity: 0, x: -50 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
            >
              <div className="relative p-1 bg-gradient-to-br from-[#004a99] to-[#0066cc] rounded-[3rem] shadow-2xl overflow-hidden group">
                <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
                <img 
                  src="/images/mtech_postgraduate.jpg" 
                  alt="M.Tech Programs" 
                  className="w-full h-[500px] object-cover rounded-[2.8rem] transition-transform duration-1000 group-hover:scale-110"
                />
                
                {/* Floating Glass Data Card */}
                <div className="absolute bottom-10 -right-10 water-glass p-8 bg-white/95 backdrop-blur-2xl border-white shadow-3xl max-w-[280px]">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 bg-[#004a99] rounded-xl flex items-center justify-center text-white">
                      <BookOpen size={24} />
                    </div>
                    <h4 className="text-lg font-black text-[#004a99] tracking-tight">Postgraduate</h4>
                  </div>
                  <p className="text-slate-500 text-sm font-medium leading-relaxed">
                    Bachelor's degree in relevant engineering branch
                  </p>
                </div>
              </div>
            </motion.div>

            {/* Right Column: Content */}
            <motion.div 
              initial={{ opacity: 0, x: 50 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              className="lg:col-span-7"
            >
              <div className="water-glass !bg-slate-50/50 p-12 md:p-20 border-white/80 shadow-[0_40px_100px_-20px_rgba(0,74,153,0.1)] relative overflow-hidden">
                <div className="absolute top-0 right-0 p-10 opacity-[0.03] pointer-events-none">
                  <BookOpen size={300} strokeWidth={1} />
                </div>

                <div className="relative z-10">
                  <div className="flex items-center gap-3 text-[#004a99] mb-8">
                    <div className="w-12 h-1 bg-[#ff6b35] rounded-full" />
                    <span className="font-black uppercase tracking-[0.2em] text-xs">Postgraduate Programs</span>
                  </div>

                  <h2 className="text-5xl md:text-7xl font-black text-slate-900 mb-10 tracking-tighter leading-[0.9]">
                    M.E. / M.Tech <br />
                    <span className="text-[#004a99]">Programs</span>
                  </h2>

                  <div className="space-y-6 text-slate-600 text-lg font-medium leading-relaxed max-w-2xl">
                    <p className="relative pl-8 border-l-4 border-[#ff6b35]">
                      Candidates must have completed B.E./B.Tech degree in relevant engineering branch with minimum required percentage.
                    </p>
                  </div>

                  {/* Requirements Checklist */}
                  <div className="mt-10 space-y-4">
                    {[
                      'B.E./B.Tech degree in relevant engineering branch',
                      'Minimum 50% aggregate marks (45% for reserved categories)',
                      'Valid TANCET or GATE score',
                      'Degree from AICTE approved institution',
                      'No age limit for M.E./M.Tech admissions'
                    ].map((req, i) => (
                      <motion.div
                        key={i}
                        initial={{ opacity: 0, x: -20 }}
                        whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: i * 0.1 }}
                        className="flex items-start gap-4"
                      >
                        <div className="w-6 h-6 rounded-full bg-[#004a99]/10 flex items-center justify-center flex-shrink-0 mt-1">
                          <CheckCircle size={16} className="text-[#004a99]" />
                        </div>
                        <p className="text-slate-700 font-medium">{req}</p>
                      </motion.div>
                    ))}
                  </div>
                </div>
              </div>
            </motion.div>

          </div>
        </div>
      </section>

      {/* Important Information Section */}
      <section className="py-40 bg-[#004a99] relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-full opacity-10">
           <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
                <path d="M 40 0 L 0 0 0 40" fill="none" stroke="white" strokeWidth="0.5"/>
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#grid)" />
          </svg>
        </div>
        
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center mb-20">
            <motion.h3 
              initial={{ opacity: 0, scale: 0.9 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              className="text-4xl md:text-6xl font-black text-white tracking-tighter mb-6"
            >
              Important <span className="text-[#ff6b35]">Information</span>
            </motion.h3>
            <p className="text-white/70 text-xl font-medium max-w-2xl mx-auto">
              Key points to remember for admission process
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: FileText, title: 'Entrance Exams', desc: 'TNEA counseling for Tamil Nadu students, JEE Main accepted' },
              { icon: Users, title: 'Reservations', desc: 'Government reservation policy applicable as per norms' },
              { icon: Award, title: 'Merit Basis', desc: 'Admissions based on entrance exam ranks and cutoffs' },
              { icon: AlertCircle, title: 'Document Verification', desc: 'Original certificates required during admission' },
              { icon: GraduationCap, title: 'Transfer Certificate', desc: 'TC from previous institution mandatory' },
              { icon: CheckCircle, title: 'Medical Fitness', desc: 'Medical fitness certificate required' }
            ].map((item, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className="water-glass !bg-white/10 p-8 border-white/20 hover:bg-white/20 transition-all group"
              >
                <div className="w-14 h-14 rounded-xl bg-[#ff6b35] flex items-center justify-center text-white mb-6 group-hover:scale-110 transition-transform">
                  <item.icon size={28} />
                </div>
                <h4 className="text-xl font-black text-white mb-3">{item.title}</h4>
                <p className="text-white/60 text-sm font-medium">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Call to Action */}
      <section className="py-24 bg-white">
        <div className="container mx-auto px-6 text-center">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          >
            <h3 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight">
              Ready to <span className="text-[#004a99]">Apply</span>?
            </h3>
            <p className="text-slate-600 text-lg font-medium mb-10 max-w-2xl mx-auto">
              Check if you meet the eligibility criteria and start your journey towards engineering excellence at ESCET.
            </p>
            <div className="flex flex-wrap gap-6 justify-center">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="px-12 py-5 bg-[#004a99] text-white rounded-2xl text-sm font-black uppercase tracking-[0.2em] hover:bg-[#003d7a] transition-all shadow-lg"
                onClick={() => window.dispatchEvent(new Event('openEnquiryModal'))}
              >
                Apply Now
              </motion.button>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="px-12 py-5 bg-white border-2 border-[#004a99] text-[#004a99] rounded-2xl text-sm font-black uppercase tracking-[0.2em] hover:bg-slate-50 transition-all"
              >
                Download Brochure
              </motion.button>
            </div>
          </motion.div>
        </div>
      </section>
    </div>
  );
};

export default EligibilityCriteria;
